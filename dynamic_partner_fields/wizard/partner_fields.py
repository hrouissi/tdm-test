...
class PartnerDynamicFields(models.TransientModel):
    _name = 'partner.dynamic.fields'
    ...
    def set_domain(self):
        ...
        model_id = self.env['ir.model'].sudo().search([('model', '=', 'res.partner')])
    ...
    position_field = fields.Many2one('ir.model.fields', string='Field Name',
                                     ...
                                     help="Note: Choosing the Fields 'Phone', 'Email', 'Mobile' and 'Address' as Position"
                                          " field may displace the newly created field. ")
    ...
    widget = fields.Many2one('partner.field.widgets', string='Widget')
    groups = fields.Many2many('res.groups', 'partner_dynamic_fields_group_rel', 'field_id', 'group_id')
